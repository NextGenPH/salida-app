...
interface FilterBarProps {
  onFilterChange: (filters: { genre: string; country: string; year: string; sortBy: string }) => void;
}
...
export const FilterBar = ({ onFilterChange }: FilterBarProps) => {
  ...
  const handleFilter = () => {
    onFilterChange({ genre, country, year, sortBy });
  };
  ...
  return (
    <div className="flex flex-wrap gap-4 p-5 md:px-10 mb-4 bg-[#141414] border-b border-gray-800">
      ...
      <select onChange={(e) => setSortBy(e.target.value)} className="bg-gray-800 text-white p-2 rounded">
